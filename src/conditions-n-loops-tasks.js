...
function isIsoscelesTriangle(a, b, c) {
  return a + b > c && a + c > b && a + c > b && (a === b || b === c || a === c);
}
...
function convertToRomanNumerals(num) {
  ...
  return romanDecs[decs] + romanNums[num % 10];
}
...
function convertNumberToString(numberStr) {
  let str = '';
  const convertOneNumToStr = (number) => {
    switch (number) {
      case '1':
        return 'one';
  ...
  const convertFullNum = (num, i = 0) => {
    str += convertOneNumToStr(num[i]);
    if (i + 1 === num.length) return str;
    str += ' ';
    return convertFullNum(num, i + 1);
  };
  return convertFullNum(numberStr);
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    if (getSum(0, i) === getSum(i + 1, arr.length)) return i;
  }
  return -1;
}
...
function getSpiralMatrix(size) {
  ...
  while (firstCol <= lastCol && firstRow <= lastRow) {
    ...
    lastCol -= 1;
    for (let i = lastCol; i >= firstCol; i -= 1) {
      matrix[lastRow][i] = num;
      num += 1;
    }
    lastRow -= 1;
    for (let i = lastRow; i >= firstRow; i -= 1) {
      matrix[i][firstCol] = num;
      num += 1;
    }
    firstCol += 1;
  }
